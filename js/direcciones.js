direccionesModulo = (function() {
  ...
  function inicializar() {
    ...
    this.mostradorDirecciones = new google.maps.DirectionsRenderer({
      draggable: true,
      map: mapa,
  ...
  function calcularYMostrarRutas() {
    ...
    var hasta = $("#hasta").val();
    var desde = $("#desde").val();
    var puntosIntermedios = obtenerPuntosIntermedios();
    var medioTrasporte = obtenerMedioTransporte();
    var request = { 
      origin: desde, 
      destination: hasta, 
    ...
    this.servicioDirecciones.route(request, function(resultado, status) {
      if (status == "OK") {
        self.mostradorDirecciones.setDirections(resultado);
      }
    });
  }
...
function obtenerMedioTransporte() {
  var medioTrasporte = $('#comoIr').val();
...
function obtenerPuntosIntermedios() {
  arrayResultado = [];
  var destinos = $('#puntosIntermedios').val();
  for (var index = 0; index < destinos.length; index++) {
    arrayResultado.push({ location:destinos[index] , stopover:true});
  }
